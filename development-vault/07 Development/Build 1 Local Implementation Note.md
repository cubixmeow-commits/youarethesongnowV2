...
- Replicate `prunaai/p-image-edit` is the selected one-cent portrait-development model. With `IMAGE_PROVIDER=replicate` and `REPLICATE_IMAGE_MODEL=prunaai/p-image-edit`, it receives one or two owner-authorized portraits as short-lived, sub-256KB data URIs and performs the scene generation in one step. Replicate states that this model's inputs and outputs are not retained or used for training.
