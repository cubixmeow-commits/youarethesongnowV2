...
1. **Private Development Build 1 authorized:** prepare the Cursor implementation handoff from the approved feature, Song DNA, API, onboarding, deployment and acceptance contracts. External beta, live Stripe and commercial protected-lyrics use remain blocked.
2. **Acceptance-test contract approved:** the complete web-journey, quality, reliability, billing, security, privacy, operations, Flutter API, accessibility and responsive-interface gates are owner-approved. The final quality set contains 90 generations, and at least 81 must preserve identity and be usable.
3. **Cursor Build 1 ready:** the music-led visual direction is approved and the authoritative Cursor handoff is at repository root. Begin the private vertical slice and verify it locally.
...
1. Build the private V2 vertical slice from `CURSOR-BUILD-1.md`.
2. Verify the local application, migrations, tests and primary browser journey.
3. Install protected test credentials only through local environment configuration.
4. Add owner-written, verified public-domain and indie-artist-authorized fixtures alongside private internal tests.
5. Use the working build to complete provider, style, quality and credit benchmarks.
